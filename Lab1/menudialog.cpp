...
void MenuDialog::availableRecipeSelected()
{
...
//             if(done) break;
//             }
//        } // WORKS TOO BUT long code
    if(!ingredientsVector.isEmpty())
    {
        for(int j=0;j<recipesToCook[selected.first().row()].getRecipeIngredients().size();j++){
            if(ingredientsVector.contains(recipesToCook[selected.first().row()].getRecipeIngredients()[j]))
            {
                int index= ingredientsVector.indexOf(recipesToCook[selected.first().row()].getRecipeIngredients()[j]);
                if(index!=(-1))
                {
                    ingredientsVector[index].setIngredientQuantity(ingredientsVector[index].getIngredientQuantity() - recipesToCook[selected.first().row()].getRecipeIngredients()[j].getIngredientQuantity());
                    if(ingredientsVector[index].getIngredientQuantity()<=0)
                        ingredientsVector.removeAt(index);
                }
            }
